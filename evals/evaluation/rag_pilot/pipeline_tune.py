...
from components.constructor.connector import get_active_pipeline, get_ragqna, reindex_data, update_active_pipeline
...
from components.tuner.base import ContentType, QuestionType, RagResult
    ...
    try:
        ...
        for item in data:
            ...
            if query and response:
                qa_list.append({"query": query, "ground_truth": ground_truth})
                ragresults.append(
                    RagResult(query=query, ground_truth=ground_truth, response=response, contexts=contexts)
                )
...
def main():
    ...
    parser.add_argument(
        "-q",
        "--qa_list_json",
        default="configs/qa_list_sample.json",
        type=str,
        help="Path to the JSON file containing the list of queries.",
    )

    # offline
    parser.add_argument("--offline", action="store_true", help="Run offline mode. Default is online mode.")
    parser.add_argument(
        "-c",
        "--rag_pipeline_json",
        default="configs/rag_pipeline_sample.json",
        type=str,
        help="Path to the JSON file containing the list of queries.",
    )
    parser.add_argument(
        "-r",
        "--ragresults_json",
        default="configs/rag_results_sample.json",
        type=str,
    ...
    for qa in qa_list:
        ...
        while True:
            ...
            if not ragqna or not active_pl:
                if not ragqna:
                    print("Error: RAG result is invalid")
                if not active_pl:
                    print("Error: RAG pipeline is invalid")
                break
