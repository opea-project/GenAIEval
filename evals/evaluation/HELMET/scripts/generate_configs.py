...
master_mapping = {
    ...
    "ruler_niah_s_1": {  # NIAH Repeat
        k: {
            ...
            "test_files": f"data/ruler/niah_single_1/validation_{v}.jsonl",
        }
    ...
    "ruler_niah_s_2": {  # NIAH
        k: {
            "input_length": v,
            "generation_max_length": 50,
            "test_files": f"data/ruler/niah_single_2/validation_{v}.jsonl",
        }
        for k, v in {"4k": 4096, "8k": 8192, "16k": 16384, "32k": 32768, "64k": 65536, "128k": 131072}.items()
    },
    "ruler_niah_s_3": {  # NIAH UUID
        k: {
            "input_length": v,
            "generation_max_length": 50,
            "test_files": f"data/ruler/niah_single_3/validation_{v}.jsonl",
        }
        for k, v in {"4k": 4096, "8k": 8192, "16k": 16384, "32k": 32768, "64k": 65536, "128k": 131072}.items()
    },
    "ruler_niah_mk_1": {  # NIAH MK Essay
        k: {
            "input_length": v,
            "generation_max_length": 50,
            "test_files": f"data/ruler/niah_multikey_1/validation_{v}.jsonl",
        }
        for k, v in {"4k": 4096, "8k": 8192, "16k": 16384, "32k": 32768, "64k": 65536, "128k": 131072}.items()
    },
    "ruler_niah_mk_2": {  # NIAH MK Needle
        k: {
            "input_length": v,
            "generation_max_length": 50,
            "test_files": f"data/ruler/niah_multikey_2/validation_{v}.jsonl",
        }
    ...
    "ruler_niah_mk_3": {  # NIAH MK UUID
        k: {
            ...
            "test_files": f"data/ruler/niah_multikey_3/validation_{v}.jsonl",
        }
        for k, v in lengths_mapping.items()
    },
    "ruler_niah_mq": {  # NIAH MQ
        k: {
            "input_length": v,
            "generation_max_length": 100,
            "test_files": f"data/ruler/niah_multiquery/validation_{v}.jsonl",
        }
        for k, v in lengths_mapping.items()
    },
    "ruler_niah_mv": {  # NIAH MV
        k: {
            "input_length": v,
            "generation_max_length": 50,
            "test_files": f"data/ruler/niah_multivalue/validation_{v}.jsonl",
        }
        for k, v in lengths_mapping.items()
    },
    "ruler_cwe": {  # RULER CWE
        k: {"input_length": v, "generation_max_length": 100, "test_files": f"data/ruler/cwe/validation_{v}.jsonl"}
        for k, v in lengths_mapping.items()
    },
    "ruler_fwe": {  # RULER FEW
        k: {"input_length": v, "generation_max_length": 50, "test_files": f"data/ruler/few/validation_{v}.jsonl"}
        for k, v in lengths_mapping.items()
    },
    "ruler_vt": {  # RULER VT
        k: {"input_length": v, "generation_max_length": 50, "test_files": f"data/ruler/vt/validation_{v}.jsonl"}
        for k, v in lengths_mapping.items()
    },
    "ruler_niah_qa_1": {  # SQuAD
        k: {"input_length": v, "generation_max_length": 50, "test_files": f"data/ruler/qa_1/validation_{v}.jsonl"}
        for k, v in lengths_mapping.items()
    },
    "ruler_niah_qa_2": {  # HotpotQA
        k: {"input_length": v, "generation_max_length": 50, "test_files": f"data/ruler/qa_2/validation_{v}.jsonl"}
        for k, v in lengths_mapping.items()
    },
    "json_kv": {
        k: {
            "input_length": v,
            "generation_max_length": 100,
            "test_files": "data/json_kv/test_k" + ["50", "105", "220", "440", "900", "1800"][i] + "_dep6.jsonl",
            "demo_files": "",
        }
    ...
    "alce_asqa": {  # ASQA
        k: {
            ...
            "name_postfix": ["_8", "_30", "_75", "_165", "_345", "_700"][i],
        }
        for i, (k, v) in enumerate(lengths_mapping.items())
    },
    "alce_qampari": {  # QAMPARI
        k: {
            "input_length": v,
            "generation_max_length": 300,
            "test_files": "data/alce/qampari_eval_gtr_top2000.json",
            "demo_files": "prompts/qampari_revised.json",
            "name_postfix": ["_8", "_30", "_75", "_165", "_345", "_700"][i],
        }
        for i, (k, v) in enumerate(lengths_mapping.items())
    },
    ...
    "kilt_nq": {
        k: {
            ...
            "test_files": "data/kilt/nq-dev-multikilt_1000_k"
            + ["20", "50", "105", "220", "440", "1000"][i]
            + "_dep6.jsonl",
    ...
    "kilt_triviaqa": {
        k: {
            "input_length": v,
            "generation_max_length": 20,
            "test_files": "data/kilt/triviaqa-dev-multikilt_1000_k"
            + ["20", "50", "105", "220", "440", "1000"][i]
            + "_dep6.jsonl",
    ...
    "kilt_hotpotqa": {
        k: {
            "input_length": v,
            "generation_max_length": 20,
            "test_files": "data/kilt/hotpotqa-dev-multikilt_1000_k"
            + ["20", "50", "105", "220", "440", "1000"][i]
            + "_dep3.jsonl",
    ...
    "msmarco_rerank_psg": {
        k: {
            ...
            "test_files": "data/msmarco/test_reranking_data_k"
            + ["14", "50", "130", "285", "600", "1000"][i]
            + "_dep3.jsonl",
    ...
    "icl_trec_coarse": {
        k: {
            ...
            "test_files": "",
            "demo_files": "",
            "name_postfix": "_" + ["200", "400", "800", "1600", "3300", "6600"][i] + "shot_balance",
        }
        for i, (k, v) in enumerate(lengths_mapping.items())
    },
    "icl_trec_fine": {
        k: {
            ...
            "test_files": "",
            "demo_files": "",
            "name_postfix": "_" + ["200", "400", "800", "1600", "3200", "6400"][i] + "shot_balance",
        }
        for i, (k, v) in enumerate(lengths_mapping.items())
    },
    "icl_banking77": {
        k: {
            ...
            "test_files": "",
            "demo_files": "",
            "name_postfix": "_" + ["180", "360", "720", "1450", "2900", "5900"][i] + "shot_balance",
        }
        for i, (k, v) in enumerate(lengths_mapping.items())
    },
    "icl_clinic150": {
        k: {
            ...
            "test_files": "",
            "demo_files": "",
            "name_postfix": "_" + ["220", "440", "880", "1750", "3525", "7050"][i] + "shot_balance",
        }
        for i, (k, v) in enumerate(lengths_mapping.items())
    },
    "icl_nlu": {
        k: {
            ...
            "test_files": "",
            "demo_files": "",
            "name_postfix": "_" + ["250", "510", "1020", "2040", "4080", "8296"][i] + "shot_balance",
        }
        for i, (k, v) in enumerate(lengths_mapping.items())
    },
}
...
def helmet_configs(input_lengths=["128k"], fname_postfix=""):
    ...
    process_configs(
        ...
        max_test_samples=100,
        shots=2,
        stop_new_line=True,  # could be false but set to true so it runs faster
    )
...
def niah_configs():
    ...
    with open("configs/niah.yaml", "w") as f:
...
def ruler_all_configs():
    ...
    with open(f"configs/niah{'' if max(input_lengths) <= 2**15 else '_long'}.yaml", "w") as f:
        config = {k: ",".join([str(c[k]) for c in configs]) for k in configs[0]}
        config.update(
            {
                "use_chat_template": False,
                "max_test_samples": 100,
