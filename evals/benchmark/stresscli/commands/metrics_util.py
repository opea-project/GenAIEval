...
def write_metrics(file_path, metrics, service_name):
    ...
    with open(file_path, 'w') as f:
        for metric, value in metrics.items():
            ...
            if 'count' in metric and not 'request_duration_count' in metric:
                f.write("\n")
            # Handle TGI/TEI
            ...
            if 'http_request_duration_seconds_sum' in metric:
                ...
                if match:
                    ...
                    sum_metric_key = next((key for key in metrics if key.startswith(sum_metric_prefix) and key.endswith('}')), None)
                    count_metric_key = next((key for key in metrics if key.startswith(count_metric_prefix) and key.endswith('}')), None)
                    ...
                    if sum_metric_key and count_metric_key:
                        ...
                        logging.info(f"Match {sum_metric_key}: {metric_value} and {count_metric_key}: {count_metric_value}")
