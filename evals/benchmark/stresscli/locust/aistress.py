...
from locust.runners import STATE_CLEANUP, STATE_STOPPED, STATE_STOPPING, WorkerRunner, MasterRunner
import sys
...
def _(parser):
    parser.add_argument("--max-request", type=int, env_var="MAX_REQUEST", default=10000, 
            help="Stop the benchmark If exceed this request")
    parser.add_argument("--http-timeout", type=int, env_var="HTTP_TIMEOUT", default=3000,
            help="Http timeout before receive response")
    parser.add_argument("--bench-target", type=str, env_var="BENCH_TARGET", default="chatqnafixed",
...
#With stream=False here, Cannot get first response time,Workaround as response.elapsed.total_seconds()
        ...
        try:
            with self.client.post(url,json=reqData,stream=False, catch_response=True,
                timeout = self.environment.parsed_options.http_timeout) as resp:
    ...
    #                   reqlist.append(reqdata)
            ...
            #Consider as a failed reqest, and report to Locust statistics
