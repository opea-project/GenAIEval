...
def getReqData():
    return {"inputs": "What is Machine Learning?", "parameters": {"max_new_tokens": 17, "do_sample": True}}
...
def respStatics(environment, resp):
    ...
    tgi_metrics = {
        "total_time": int(resp.headers["x-total-time"]),
...
def staticsOutput(environment, reqlist):
    ...
    console_logger.warning(
        qtime_msg.format(numpy.percentile(queue_time, 50), numpy.percentile(queue_time, 99), numpy.average(queue_time))
    )
    console_logger.warning(
        tptk_msg.format(
            numpy.percentile(time_per_token, 50), numpy.percentile(time_per_token, 99), numpy.average(time_per_token)
        )
    )
