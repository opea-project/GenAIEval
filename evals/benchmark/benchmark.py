...
def extract_test_case_data(content):
    ...
    return {
        ...
        "all_case_data": {example: content["test_cases"].get(example, {}) for example in test_suite_config.get("examples", [])}
    }
...
def create_and_save_run_yaml(example, service_type, service_name, base_url, test_suite_config, index):
    ...
    for user_queries in test_suite_config["user_queries"]:
        ...
        bench_target = f"{example}{'bench' if service_type == 'e2e' and test_suite_config['random_prompt'] else 'fixed'}"
        run_yaml_content = create_run_yaml_content(service_name, base_url, bench_target, concurrency, user_queries, test_suite_config)
...
def run_service_test(example, service_type, service_name, parameters, test_suite_config):
    ...
    run_yaml_paths = create_and_save_run_yaml(example, service_type, service_name, base_url, test_suite_config, timestamp)
...
def process_service(example, service_name, case_data, test_suite_config):
    ...
    if service and service.get("run_test"):
        ...
        run_service_test(example, service_name, service.get("service_name"), service.get("parameters", {}), test_suite_config)
