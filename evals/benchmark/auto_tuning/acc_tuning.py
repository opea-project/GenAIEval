...
def generate_acc_strategy_files(tuning_config, strategy, output_folder):
    os.makedirs(output_folder, exist_ok=True)
    strategy_files_dict = {}
    strategy_dict = {}
    ...
    all_files_created = False
    index = 0
    for embed_model in tuning_config["embedding_models"]:
        for rank_model in tuning_config["reranking_models"]:
            for llm_model in tuning_config["llm_models"]:
                tmp_config = copy.deepcopy(strategy)
                tmp_config["embedding-dependency"]["model_id"] = embed_model
                tmp_config["reranking-dependency"]["model_id"] = rank_model
                tmp_config["llm-dependency"]["model_id"] = llm_model
                output_file = os.path.join(output_folder, f"strategy_acc_{index}.json")
                success = tuning_utils.write_json(tmp_config, output_file)
                if success:
                    all_files_created = True
                    strategy_files_dict[index] = output_file
                    strategy_dict[index] = tmp_config
                else:
                    all_files_created = False
                index += 1
...
def main():
    ...
    tuning_config = tuning_utils.load_tuning_config(args.tuning_config)
